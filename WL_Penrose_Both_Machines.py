...
def has_relevant_files(directory):
    for filename in os.listdir(directory):
        ...
        if filename.startswith('RI') and os.path.getsize(filepath) in range(2770 * 1024 - 1000, 2770 * 1024 + 1000):
            return True
    return False
...
if __name__ == "__main__":
    base_dir = r'\\10.2.169.73\va_transfer\TDS\H194581\DailyQA\6DoF Daily QA'
    base_dest_dir = r'\\ceinvfs03s\shared\PH_RAD_ONC\Everyone\PATIENTS\Stereotactic\StereoQA'
    today_date_str = datetime.now().strftime('%Y%m%d')
    today_dest_dir = os.path.join(base_dest_dir, f"{today_date_str}_TB4581")
    ...
    if today_source_dir:
        ...
        pdf_path = os.path.join(today_dest_dir, "WL_TB4581.pdf")
...
#Pause to run script on 2026 machine
        import os
import shutil
from datetime import datetime, date
from pylinac import WinstonLutz
...
if __name__ == "__main__":
    base_dir = r'\\10.2.169.73\va_transfer\TDS\H192026\DailyQA\6DoF Daily QA'
    base_dest_dir = r'\\ceinvfs03s\shared\PH_RAD_ONC\Everyone\PATIENTS\Stereotactic\StereoQA'
    today_date_str = datetime.now().strftime('%Y%m%d')
    today_dest_dir = os.path.join(base_dest_dir, f"{today_date_str}_TB2026")
    ...
    if today_source_dir:
        ...
        wl = WinstonLutz(today_dest_dir)
        wl.analyze(bb_size_mm=10)
        pdf_path = os.path.join(today_dest_dir, "WL_TB2026.pdf")
        wl.publish_pdf(pdf_path)
        print(f"Published Winston Lutz analysis PDF at: {pdf_path}")
    else:
        print(f"No suitable source directory found today in {base_dir}")
